...
const HashTagList = ({
  ...
  return (
    <label className="grid grid-cols-[1fr_3fr] gap-[2.6rem]">
      ...
      <div className="rounded-[0.8rem] border border-primary-200 bg-[#F7F6F6]  text-[1.6rem]   focus:border-primary-400 flex">
        <div className="flex space-x-[0.7rem] pl-[1rem] items-center">
          {hashTags?.map((tag, index) => (
            <div
              key={index}
              className="flex items-center Sub2 textColor  rounded-full p-[1rem] bg-primary-main h-[4rem] space-x-[0.8rem] hover:bg-[#e46655] cursor-pointer"
        ...
        <input
          className="focus:outline-none  focus:ring-transparent border-none bg-transparent p-[1.4rem_1.8rem] text-[1.6rem] flex-1 placeholder:text-primary-400"
          type="text"
